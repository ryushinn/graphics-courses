function loadOBJ(renderer, path, name, objMaterial, transform, animated = false) {
					...
					object.traverse(function (child) {
						if (child.isMesh) {
							...
							for (let i = 0; i < renderer.lights.length; ++i) {
								...
								let light = renderer.lights[i].light.entity;
								switch (objMaterial) {
									case 'PhongMaterial':
								...
								material.then((data) => {
									let meshRender = new MeshRender(renderer.gl, mesh, data, animated);
									renderer.addMeshRender(meshRender, i);
								});
								shadowMaterial.then((data) => {
									let shadowMeshRender = new MeshRender(renderer.gl, mesh, data, animated);
									renderer.addShadowMeshRender(shadowMeshRender, i);
								});
							}
						}
					});

...
class MeshRender {
	...
	animated_trans = [0.0, 0.0, 0.0];

	constructor(gl, mesh, material, animated = false) {
		// add a property "animated" for animation
	...
	bindCameraParameters(camera) {
		...
		if (this.animated) {
			mat4.translate(modelMatrix, modelMatrix, this.animated_trans);
		}
	...
	bindMaterialParameters() {
		...
		for (let k in this.material.uniforms) {
			...
			if (this.material.uniforms[k].type == 'matrix4fv') {
				let val = mat4.create();
				mat4.mul(val, val, this.material.uniforms[k].value)
				let scale = this.mesh.transform.scale;
				if (k == "uLightMVP" && this.animated) {
					mat4.scale(val, val, [1.0 / scale[0], 1.0 / scale[1], 1.0 / scale[2]]);
					mat4.translate(val, val, this.animated_trans);
					mat4.scale(val, val, scale);
				}
				gl.uniformMatrix4fv(
